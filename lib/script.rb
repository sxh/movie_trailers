...
monitor = TheMovieDb::MovieDbMonitor.minimal
MovieNas::MovieLibrary.new('/Volumes/Media/Movies').movie_directories.reject(&:has_trailer?).each do |movie_directory|
  puts '*************************************************************************'
  ...
  puts '*************************************************************************'
  TheMovieDb::Search.new(monitor, movie_directory.name.movie, movie_directory.name.year).movies.each do |movie|
